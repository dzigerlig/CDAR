...
app.controller('TemplatesController', ['$scope', '$routeParams', 'TreeService', 'AuthenticationService', 'UserService',	'$route', '$modal', 'DescriptionService',
	...
	$scope.editTemplateTitle = function(data, id) {
		if (data.length>45) {
			...
			TreeService.updateTemplate({entity1 : 'ktrees', id1 : $routeParams.treeId, id2 : template.id}, template, function(response) {
				...
				noty({
					type: 'success', 
					text : 'template renamed successfully', 
					timeout: 1500
				});
			}, function(error) {
				UserService.checkResponseUnauthorized(error);
	...
	$scope.saveTemplate = function() {
		if ($scope.selectedTemplate.id !== 0) {
			...
			TreeService.updateTemplate({
				...
				noty({
					type: 'success', 
					text : 'template text edited successfully',
					timeout: 1500
				});
			}, function(error) {
				UserService.checkResponseUnauthorized(error);
